...
public class MartController {
	...
	public void executeToMart() {
		try (Connection controlConn = JDBCUtil.getConnection(constant.CONTROL_DB);
			...
			// 2. Gửi email thông báo bắt đầu
            IJavaMail emailService = new EmailService();
            emailService.send("Bắt đầu thực hiện toMart", "Hệ thống đang xử lý...");

			// 3. Ghi log trạng thái Processing
			Log processingLog = new Log(latestLog.getConfigID(), constant.TO_MART_ACTION, "Nguyen Thi Hoai Thu",
					constant.PROCESSING_STATUS);
			insertLog(controlConn, processingLog);

			// 4. Transform dữ liệu từ warehouse_db và chèn vào mart_db
			boolean check = transformAndInsertData(warehouseConn, martConn);
			if(!check) {
				System.out.println("fail ở đây");
				Log failLog = new Log(latestLog.getConfigID(), constant.TO_MART_ACTION, "Nguyen Thi Hoai Thu",
						constant.FAIL_STATUS);
				insertLog(controlConn, failLog);
				emailService.send("Lỗi toMart", "Hệ thống đang lỗi ở Transform dữ liệu từ warehouse_db và chèn vào mart_db ...");
				return;
			}

			// 5. Ghi log trạng thái Success
			Log successLog = new Log(latestLog.getConfigID(), constant.TO_MART_ACTION, "Nguyen Thi Hoai Thu",
					constant.SUCCESS_STATUS);
			insertLog(controlConn, successLog);

			// 6. Gửi email thông báo hoàn thành
			 //IJavaMail emailService = new EmailService();
			 emailService.send("Hoàn thành toMart", "Quá trình toMart đã thành công!");
			// EmailUtil.sendEmail("admin@example.com", "Hoàn thành toMart", "Quá trình
			// toMart đã thành công!");

		} catch (SQLException e) {
			IJavaMail emailService = new EmailService();
            emailService.send("Lỗi toMart", "Hệ thống đang lỗi kết nối CSDL...");
	...
	private boolean transformAndInsertData(Connection warehouseConn, Connection martConn) throws SQLException {
		String queryWarehouse = """
				...
				""";
		// Truy vấn 2: Tính toán thống kê dựa trên game_id
		...
		try (Statement stmtWarehouse = warehouseConn.createStatement();
			...
			while (rs.next()) {
				...
				try (PreparedStatement psGame = martConn.prepareStatement(insertGameDim)) {
	                ...
	                System.out.println("lỗi khi insert vào game_dim");
			    ...
			    if (stats != null) {
			        ...
			        try (PreparedStatement psFact = martConn.prepareStatement(insertFact)) {
			        	...
			        	System.out.println("lỗi khi insert vào game_price_fact");
			     ...
			     // Insert vào game_sales_report
					try (PreparedStatement psReport = martConn.prepareStatement(insertReport,     
						...
						System.out.println("lỗi khi insert vào game_sales_report");
